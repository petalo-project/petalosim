...
void PETitPyrex::BuildBox()
{
  ...
  if (blue_tiles_) {
    G4Material* pyrex2 =
      G4NistManager::Instance()->FindOrBuildMaterial("G4_Pyrex_Glass");
    pyrex2->SetMaterialPropertiesTable(petmaterialprops::Pyrex_vidrasa());

    G4Box* panel_sipms_solid =
      new G4Box("SiPM_PANEL", panel_sipm_xy_size_/2., panel_sipm_xy_size_/2.,
                (panel_thickness_ + wls_depth_)/2.);

    G4LogicalVolume* panel_sipms_logic =
      new G4LogicalVolume(panel_sipms_solid, pyrex2, "SiPM_PANEL");

    // WAVELENGTH SHIFTER LAYER ON THE PANEL //
    G4Box* wls_solid =
      new G4Box("WLS", panel_sipm_xy_size_/2., panel_sipm_xy_size_/2.,
                wls_depth_/2);

    G4Material* wls = materials::TPB();
    wls->SetMaterialPropertiesTable(petmaterialprops::TPB());
    ...
    G4OpticalSurface* wls_optSurf =
      new G4OpticalSurface("WLS_OPSURF", glisur, ground,
                           dielectric_dielectric, .01);
    new G4LogicalSkinSurface("WLS_OPSURF", wls_logic, wls_optSurf);

    G4double panel_sipms_zpos =
      box_->GetBoxSize()/2. - box_->GetBoxThickness() - dist_dice_flange_ -
      tile_thickn_ - dist_sipms_panel_sipms_ -
      (panel_thickness_ + wls_depth_)/2.;
    new G4PVPlacement(0, G4ThreeVector(0., 0., -panel_sipms_zpos),
                      panel_sipms_logic, "SiPM_PANEL", active_logic_,
                      false, 1, false);

    G4RotationMatrix rot_panel;
    rot_panel.rotateY(pi);

    new G4PVPlacement(G4Transform3D(rot_panel,
                                    G4ThreeVector(0., 0., panel_sipms_zpos)),
                      panel_sipms_logic, "SiPM_PANEL", active_logic_,
                      false, 2, false);
