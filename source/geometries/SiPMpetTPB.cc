...
void SiPMpetTPB::Construct()
{
  ...
  if (phys_)
  {
    ...
    TPB->SetMaterialPropertiesTable(petmaterialprops::TPB_LXe(decay_time_));
  }
  else
  {
    G4cout << "TPB with constant refraction index equal to 1.7 " << G4endl;
    TPB->SetMaterialPropertiesTable(petmaterialprops::TPB_LXe_nconst(decay_time_));
  }
