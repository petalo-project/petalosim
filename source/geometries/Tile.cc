...
void Tile::Construct()
{
  ...
  for (int i = 0; i < n_columns_; i++)
  {
    ...
    {
      count += 1;
      new G4PVPlacement(0, G4ThreeVector(-tile_x_ / 2. + offset_x + sipm_dim.x() / 2. + i * sipm_pitch_, -tile_y_ / 2. + offset_y + sipm_dim.y() / 2. + j * sipm_pitch_, tile_z_ / 2. - air_z + sipm_dim.z() / 2.),
